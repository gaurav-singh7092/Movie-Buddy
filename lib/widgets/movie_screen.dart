...
class _MovieScreenState extends State<MovieScreen> {
  int selected = 0;
  List<String> category = ['Trending Movies', 'In Theatre', 'Top Rated'];
  List trendingMovie = [];
  List topRatedMovie = [];
  List theatre = [];
  ...
  loadMovie() async {
    ...
    Map theatreResult = await tmdbwithcustomlogs.v3.movies.getNowPlaying();
    setState(() {
      trendingMovie = trendingresult['results'];
      topRatedMovie = topRatedResult['results'];
      theatre = theatreResult['results'];
    });
  }
  @override
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          if(selected == 0)
            MovieCard(result: trendingMovie)
          else if(selected == 1)
            MovieCard(result: theatre)
          else if(selected==2)
              MovieCard(result: topRatedMovie)
        ],
      ),
    );
  }
